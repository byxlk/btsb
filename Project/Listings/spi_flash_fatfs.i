...
#line 20 "..\\User\\spi_flash_fatfs.c"
#line 21 "..\\User\\spi_flash_fatfs.c"
#line 1 "..\\User\\spi_flash_fatfs.h"
...
#line 22 "..\\User\\spi_flash_fatfs.c"

 


uint8_t g_TestBuf[(4*1024)];
...
static void CreateNewFile(void)
{
	...
	result = f_open(&file, "armfly.txt", 0x08 | 0x02);
...
static void ReadFileData(void)
{
	...
	result = f_open(&file, "armfly.txt", 0x00 | 0x01);
	if (result !=  FR_OK)
	{
...
#line 553 "..\\User\\spi_flash_fatfs.c"
...
static void WriteFileTest(void)
{
	...
	result = f_open(&file, TestFileName, 0x08 | 0x02);

	 
	printf("开始写文件%s %dKB ...\r\n", TestFileName, (2*1024*1024) / 1024);
	runtime1 = bsp_GetRunTime();	 
	for (i = 0; i < (2*1024*1024) / (4*1024); i++)
	{
	...
	if (err == 0)
	{
		...
		printf("\r\n  写耗时 : %dms   平均写速度 : %dB/S (%dKB/S)\r\n",
			timelen,
			((2*1024*1024) * 1000) / timelen,
			(((2*1024*1024) / 1024) * 1000) / timelen);
	}
	...
	result = f_open(&file, TestFileName, 0x00 | 0x01);
	if (result !=  FR_OK)
	{
		printf("没有找到文件: %s\r\n", TestFileName);
		return;
	...
	if (err == 0)
	{
		...
		printf("\r\n  读耗时 : %dms   平均读速度 : %dB/S (%dKB/S)\r\n", timelen,
			((2*1024*1024) * 1000) / timelen, (((2*1024*1024) / 1024) * 1000) / timelen);
	}
