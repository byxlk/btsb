...
#line 19 "..\\User\\bsp\\bsp_cpu_rtc.c"
...
void bsp_InitRTC(void)
{
     ...
     RCC_APB1PeriphClockCmd(((uint32_t)0x10000000) | ((uint32_t)0x00040000), ENABLE);
    ...
    if (RTC_ReadBackupRegister(((uint32_t)0x00000001)) != 0xA5A5)
    {
        
	 
	RTC_InitStructure.RTC_AsynchPrediv = 0x7F;
	RTC_InitStructure.RTC_SynchPrediv =  0xFF;
	RTC_InitStructure.RTC_HourFormat = ((uint32_t)0x00000000);
	RTC_Init(&RTC_InitStructure);
        ...
        bsp_RTC_SetDate(2017, ((uint8_t)0x01), 1);
        bsp_RTC_SetTime(8, 58, 0);

         
        RTC_WakeUpClockConfig(((uint32_t)0x00000000));
        RTC_SetWakeUpCounter(0x7FF);

         
        RTC_ITConfig(((uint32_t)0x00004000), ENABLE);
 
         
        RTC_WakeUpCmd(ENABLE);
		
         
        RTC_WriteBackupRegister(((uint32_t)0x00000001), 0xA5A5);
    }
    else
    {
        ...
        RTC_WaitForSynchro();
        RTC_ClearITPendingBit(((uint32_t)0x00004000));
...
void bsp_RTC_SetTime(uint8_t _hour, uint8_t _min, uint8_t _sec)
{
	...
	RTC_TimeStructure.RTC_Hours   = ((_hour) <= 23)? _hour : 0x08;
	RTC_TimeStructure.RTC_Minutes = ((_min) <= 59)? _min : 0x00;
	RTC_TimeStructure.RTC_Seconds = ((_sec) <= 59)? _sec : 0x00;
    
	 
	RTC_SetTime(((uint32_t)0x000000000), &RTC_TimeStructure);  
}
...
void bsp_RTC_SetDate(uint16_t _year, uint8_t _mon, uint8_t _day)
{
	...
	RTC_DateStructure.RTC_Year = ((_year - 2000) <= 99)? (_year - 2000) : 16; 
	RTC_DateStructure.RTC_Month = (((_mon) >= 1) && ((_mon) <= 12))? _mon : ((uint8_t)0x01);
	RTC_DateStructure.RTC_Date = (((_day) >= 1) && ((_day) <= 31))? _day : 0x01;  
	RTC_DateStructure.RTC_WeekDay = bsp_RTC_CalcWeek(RTC_DateStructure.RTC_Year,
                                                                                                  RTC_DateStructure.RTC_Month,
                                                                                                  RTC_DateStructure.RTC_Date); 
    
	 
	RTC_SetDate(((uint32_t)0x000000000), &RTC_DateStructure);
}
...
uint32_t bsp_RTC_GetSecond(uint16_t _year, uint8_t _mon, uint8_t _day, uint8_t _hour, uint8_t _min, uint8_t _sec)
{
	...
	for (t = 1970; t < _year; t++) 	 
	{
		if (IS_RTC_LeapYear(t))		 
		{
			seccount += 31622400;	 
		}
		else
	...
	seccount += (uint32_t)(_day - 1) * 86400;	 

	seccount += (uint32_t)_hour * 3600;		 
...
void bsp_RTC_ReadClock(RTC_DateTypeDef *pDate, RTC_TimeTypeDef   *pTime)
{
    ...
    if(! pDate)
        RTC_GetDate(((uint32_t)0x000000000), pDate);

    if(! pTime)
        RTC_GetTime(((uint32_t)0x000000000), pTime);
}   
...
void bsp_RTC_GetClock(void)
{
    ...
    RTC_GetDate(((uint32_t)0x000000000), &RTC_DateStructure);
    RTC_GetTime(((uint32_t)0x000000000), &RTC_TimeStructure);
