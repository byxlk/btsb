...
void bsp_InitDWT(void)
{
	*(volatile unsigned int *)0xE000EDFC         |= (unsigned int)(1 << 24);    
	*(volatile unsigned int *)0xE0001004      = (unsigned int)0u;
	*(volatile unsigned int *)0xE0001000         |= (unsigned int)(1 << 0);
}
...
void bsp_DelayUS(uint32_t _ulDelayTime)
{
	...
	tStart = *(volatile unsigned int *)0xE0001004;                                      
	tCnt = 0;
	tDelayCnt = _ulDelayTime * (SystemCoreClock / 1000000);	   		      

	while(tCnt < tDelayCnt)
	{
		tCnt = *(volatile unsigned int *)0xE0001004 - tStart;  	
	}
}
