...
*********************************************************************************************************
*	函 数 名: NMI_Handler
*	功能说明: 不可屏蔽中断服务程序。Cortex-M3 内核异常中断服务程序。
*	形    参: 无
*	返 回 值: 无
*********************************************************************************************************
*/
...
void RTC_IRQHandler(void)
{
	...
	It_Debug("--R--%d %d %d\n\r",RTC_GetITStatus(RTC_IT_TS),RTC_GetITStatus(RTC_IT_ALRA),RTC_GetWakeUpCounter());

	if(RTC_GetITStatus(RTC_IT_ALRA) != RESET) //闹钟报警
	{
		It_Debug("RTC Alarm!\n\r");
	...
	if(RTC_GetITStatus(RTC_IT_TS) != RESET)
	{
	    ...
	    //实时事件处理
		if(RTC_GetWakeUpCounter()==RTC_Counter)
		{
			RTC_Counter+=60;
	    ...
	    RTC_ClearITPendingBit(RTC_IT_TS);/* Clear the RTC Second interrupt */
	}
	...
	if(ItemsTotal)
	{
		...
		EventParam.Num=(int)RTC_GetWakeUpCounter();
