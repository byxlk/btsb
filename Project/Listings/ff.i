...
)
{
	...
	for (;;) {
		...
		if (((BYTE)(w) >= 0x81 && (BYTE)(w) <= 0xFE)) {				 
			b = (BYTE)p[si++];			 
			w = (w << 8) + b;			 
			if (!(((BYTE)(b) >= 0x40 && (BYTE)(b) <= 0x7E) || ((BYTE)(b) >= 0x80 && (BYTE)(b) <= 0xFE))) return FR_INVALID_NAME;	 
		}
		w = ff_convert(w, 1);			 
